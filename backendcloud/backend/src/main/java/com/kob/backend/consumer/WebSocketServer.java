...
public class WebSocketServer {
    ...
    private User user;
    public static ConcurrentHashMap<Integer,WebSocketServer> users = new ConcurrentHashMap<>();//链接池子，存储所有的链接
    ...
    public static void startGame(Integer aId,Integer aBotId,Integer bId,Integer bBotId){//新匹配系统
    ...
    public void startMatching(Integer bot_id){//匹配系统
        MultiValueMap<String,String> data = new LinkedMultiValueMap<>();
        data.add("user_id", this.user.getId().toString());
    ...
    public void move(Integer direction){
        if(game.getPlayerA().getId().equals(user.getId())){
            if(game.getPlayerA().getBotId().equals(-1))//特判只有亲自出马才使用setNextStep来人工操作
    ...
    @OnMessage
    public void onMessage(String message, Session session) {//接收函数充当路由的作用
        // 从Client接收消息
